//controlled by mouse and arrow keys
...
function monster(x, y, r, g, b,scalex) {
  scale(scalex);
  rectMode(CORNER);
...
function monster2(x, y, r, g, b, scalex) {
  scale(scalex);
  //body
  fill(r, g, b);
...
function draw() {
  ...
  // CONTROLS AUTOMATIC HORIZONTAL MOVEMENT
  if (moveX < 300 && xChecker == true) {
    moveX += 2;
  } else if (moveX > 0) {
    xChecker = false
    moveX -= 2;
  ...
  // CONTROLS AUTOMATIC VERTICAL MOVEMENT
  if (moveY < 100 && yChecker == true) {
    moveY += 2;
  } else if (moveY > 0) {
    yChecker = false
    moveY -= 2;
  ...
  monster2(300 + moveX, 300 + moveY, 50, 60, 70, 1);

  monster(450 - mouseX / 2, 200 - mouseY / 2, n, c, 0,1);

  monster2(2 * mouseX, 2 * mouseY - moveY * 2, 150, 30, 120, .5);

  monster(n, c, mouseX / 3, mouseY / 3, mouseX,1);
